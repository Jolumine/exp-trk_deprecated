from PyQt5.QtWidgets import QComboBox, QDialog, QPushButton, QLabel, QVBoxLayout
from PyQt5.QtGui import QIcon

from ...vars import Delete_Logo, log_file
...
class Delete_Window(QDialog):
    def __init__(self, active, parent=None):
        ...
        with open(self.active_user_path, "r") as file: 
            ...
            for k in self.parsed["passive income"]: 
                self.all.append(f"{k}-{self.parsed['passive income'][k]['Amount']}-{self.parsed['passive income'][k]['Repeated']}")
    ...
    def delete(self):
        ...
        if splitted[0] in self.parsed["passive income"]:
